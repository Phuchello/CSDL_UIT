...
for (const name of pdfs) {
  ...
  const targetLower = resolve(destination, name.toLowerCase())
  const targetExact = resolve(destination, name)
  if (!existsSync(source)) throw new Error(`Missing frozen PDF: ${source}`)
  copyFileSync(source, targetLower)
  copyFileSync(source, targetExact)
  const sourceHash = sha256(source)
  const targetHash = sha256(targetLower)
