...
def check_alias_columns(snippet: str, tables: dict[str, set[str]], pseudo_tables: dict[str, str] | None = None) -> None:
    aliases = dict(re.findall(r"(?:FROM|JOIN)\s+dbo\.(tr_\w+)\s+AS\s+(\w+)", snippet, re.I))
    if pseudo_tables:
    ...
    for alias, column in re.findall(r"\b(\w+)\.(\w+)", snippet):
...
def main() -> int:
    ...
    print(f"LAB03 FIXTURE CONTRACT: {'PASS' if not any('Lab 03' in x or 'canonical table set' in x or 'unknown canonical table' in x or 'unknown column' in x or 'order fixture' in x or 'expected result' in x for x in failures) else 'FAIL'}")
    print(f"TRIGGER FIXTURE CONTRACT: {'PASS' if not any('Trigger' in x or 'trigger' in x or 'unknown column' in x for x in failures) else 'FAIL'}")
